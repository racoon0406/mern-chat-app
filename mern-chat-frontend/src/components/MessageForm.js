...
function MessageForm() {
    ...
    //scrollToBottom every time we send a message
    useEffect(() =>{
        scrollToBottom();
    ...
    socket.off('room-messages').on('room-messages', (roomMessages) => { //make sure to switch off before switch on
        //console.log(roomMessages);
        setMessages(roomMessages);//save roomMessages from backend
    })
    ...
    function handleSubmit(e){
        ...
        socket.emit('message-room', roomId, message, user, time, todayDate); //create a message and refresh room messages
        setMessage('');//reset
    }
